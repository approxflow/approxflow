...
if __name__ == "__main__":
  with open("GuessPresenceSingleHouse.c.param", 'r') as f:
    s = f.read()
  #for N in xrange(3,17):
  # for N in 36, 49, 64, ... up to 17^2
  for N in [x*y for (x,y) in zip(xrange(6, 17), xrange(6,17))]:
    for mode in ['S', 'M', 'L']:
      for case in [1, 2]:
        cfilename = "GuessPresenceSingleHouse.N" + str(N) + ".mode" + str(mode) + ".case" + str(case) + ".c"
        with open(cfilename, "w+") as f:
          f.write("#define CONST_N " + str(N) + "\n#define CONST_MODE \'" + str(mode) + "\'\n#define CONST_CASE " + str(case) + "\n\n" + s)
