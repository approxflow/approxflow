...
if __name__ == "__main__":
  with open("PreferenceRanking.c.param", 'r') as f:
    s = f.read()
  #for N in [x*y for (x,y) in zip(xrange(3, 10), xrange(3,10))]:
  for N in xrange(3,10,2):
    for C in xrange(3,10,2):
      cbmc_unwind_limit = 16
      cfilename = "PreferenceRanking.N" + str(N) + ".C" + str(C) + ".c"
      with open(cfilename, "w+") as f:
        f.write("#define CONST_N " + str(N) + "\n#define CONST_C " + str(C) + "\n\n" + s)
  
      cnf_filename = cfilename + ".unwind" + str(cbmc_unwind_limit) + ".cnf"

      p = subprocess.Popen(["cbmc"] + [cfilename, "--32", "--dimacs", "--unwind", str(cbmc_unwind_limit), "--outfile", cnf_filename], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
      out, err = p.communicate()

      allsat_vars = None
      with open(cnf_filename, "r") as cnffile:
        cnfstr = cnffile.read()
      with open(cnf_filename, "r") as cnffile:
      ...
      i = 0
      commentstr = ""
      with open(cnf_filename, "w+") as cnffile:
      ...
      with open(cnf_filename + ".scope", "w+") as scopefile:
