...
def sort_clauses(filename):
  external_tool = os.path.join(os.path.dirname(os.path.realpath(__file__)), "vegard-cnf-utils", "cnf-sort-clauses")
  args = filename
  new_filename = None
  with tempfile.NamedTemporaryFile(delete=False) as cnf_new:
    ...
    try:
      ...
      print "Error: external tool " + tool_name + " not found, not performing clause sorting"
      return
...
def get_argparser():
  ...
  parser = argparse.ArgumentParser(description='Order the clauses in ascending order. (courtesy of Vegard Nossum, https://github.com/vegard/cnf-utils)')
