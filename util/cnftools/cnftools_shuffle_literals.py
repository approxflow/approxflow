...
def shuffle(filename):
  external_tool = os.path.join(os.path.dirname(os.path.realpath(__file__)), "vegard-cnf-utils", "cnf-shuffle-literals")
  args = filename
  new_filename = None
  with tempfile.NamedTemporaryFile(delete=False) as cnf_new:
    ...
    try:
      ...
      print "Error: external tool " + tool_name + " not found, not performing literal shuffling"
      return
...
def get_argparser():
  ...
  parser = argparse.ArgumentParser(description='Shuffle the order of the literals within each clause, but leave the order of the clauses intact (courtesy of Vegard Nossum, https://github.com/vegard/cnf-utils)')
