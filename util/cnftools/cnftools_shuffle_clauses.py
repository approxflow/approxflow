...
def shuffle(filename):
  external_tool = os.path.join(os.path.dirname(os.path.realpath(__file__)), "vegard-cnf-utils", "cnf-shuffle-clauses")
  args = filename
  new_filename = None
  with tempfile.NamedTemporaryFile(delete=False) as cnf_new:
    ...
    try:
      ...
      print "Error: external tool " + tool_name + " not found, not performing clause shuffling"
      return
...
def get_argparser():
  ...
  parser = argparse.ArgumentParser(description='Shuffle the order of the clauses, but leave the order of literals within each clause intact. (courtesy of Vegard Nossum, https://github.com/vegard/cnf-utils)')
...
def get_mode_names():
  ...
  second = ["clauses", "c", "clause", "cl"]
  separators = ["_", "-"]
  prod = list(product(first, second))
  alias_list = []
  main_name = "shuffle_clauses"
